...
interface TabTriAllianceProps {
  accountName: string | null
  serverNumber: number | null
}

export default function TabTriAlliance({ accountName, serverNumber }: TabTriAllianceProps) {
  ...
  const loadTriAllianceData = useCallback(async () => {
    if (!selectedAlliance) return
    const triAllianceRes = await api.getTriAlliance(
      selectedAlliance.owner_account,
      selectedAlliance.owner_server,
      selectedAlliance.slug
    )
    if (triAllianceRes.ok && triAllianceRes.data) {
      const legs = (triAllianceRes.data.legions ?? [
        { name: 'Legion 1', member_ids: [], filler_ids: [] },
        { name: 'Legion 2', member_ids: [], filler_ids: [] },
      ...
      setAttendanceRecords(triAllianceRes.data.attendance_records ?? [])
    }
  }, [selectedAlliance])
  ...
  useEffect(() => {
    if (selectedAlliance) loadTriAllianceData()
    else {
      setLegions([
        { name: 'Legion 1', member_ids: [], filler_ids: [] },
        { name: 'Legion 2', member_ids: [], filler_ids: [] },
  ...
  }, [selectedAlliance, loadTriAllianceData])
  ...
  const assignToLegion = async (playerId: string, legionIndex: number | null) => {
    ...
    setLegions(next)
    setSaving(true)
    const { ok } = await api.setTriAllianceLegions(
      selectedAlliance.owner_account,
      selectedAlliance.owner_server,
      selectedAlliance.slug,
      next
  ...
  const submitAttendance = async () => {
    ...
    const { ok } = editingRecordId
      ? await api.updateTriAllianceAttendance(
          selectedAlliance.owner_account,
          selectedAlliance.owner_server,
          selectedAlliance.slug,
          editingRecordId,
      ...
      : await api.addTriAllianceAttendance(
          selectedAlliance.owner_account,
          selectedAlliance.owner_server,
          selectedAlliance.slug,
          payload
    ...
    <div>
      <div className="text-center mb-8">
        <div className="inline-block bg-indigo-900/50 rounded-full p-4 mb-4">
          <i className="fas fa-users-cog text-indigo-400 text-3xl"></i>
        </div>
        <h2 className="text-3xl font-bold text-white mb-2">Tri Alliance</h2>
          ...
          <div className="bg-gray-800 rounded-xl shadow-2xl border border-gray-700 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <h3 className="text-xl font-bold text-white mb-4">
                {editingRecordId ? 'Edit attendance record' : 'Record attendance'}
              </h3>
              <div className="space-y-4 mb-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Tri Alliance Week 1"

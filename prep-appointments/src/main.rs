use prep_appointments::display::{print_day_schedule, write_schedule_to_file};
use prep_appointments::parser::load_appointments;
use prep_appointments::schedule::{
    schedule_construction_day, schedule_research_day, schedule_troops_day,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Loaded {} appointment entries (resubmissions merged)",
        entries.len()
    );
    ...
    print_day_schedule("Construction Day", &construction_schedule, &entries, |e| {
        e.construction_score
    });
    print_day_schedule("Research Day", &research_schedule, &entries, |e| {
        e.research_score
    });
    print_day_schedule("Troops Training Day", &troops_schedule, &entries, |e| {
        e.troops_speedups
    });

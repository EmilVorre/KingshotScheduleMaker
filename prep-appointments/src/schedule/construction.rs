use super::move_chain::{apply_move_chain, find_move_chain};
...
pub fn schedule_construction_day_with_locked(
    ...
    last_slot_priority.sort_by(|a, b| b.construction_score.cmp(&a.construction_score));

    // Sort other candidates by construction score (highest first)
    other_candidates.sort_by(|a, b| b.construction_score.cmp(&a.construction_score));
    ...
    for entry in remaining_candidates {
        ...
        if !assigned {
                    ...
                    if let Some(appt) = schedule.get(slot) {
                        if let Some(blocking_entry) = entry_map.get(&appt.player_id) {
                            ...
                            let combined_score = if *slot == last_slot {
                                ...
                                if blocking_entry.wants_research
                                    && blocking_entry.research_available_slots.contains(&1)
                                {
            ...
            for (requested_slot, _blocking_player_id, _blocking_score, _combined_score) in
                &blocking_players
            {
                ...
                if *requested_slot == last_slot {
                    if let Some(blocking_appt) = schedule.get(requested_slot) {
                        if let Some(blocking_entry) = entry_map.get(&blocking_appt.player_id) {
                            ...
                            let requester_combined = if entry.wants_research
                            ...
                            let holder_combined = if blocking_entry.wants_research
                                && blocking_entry.research_available_slots.contains(&1)
                            {

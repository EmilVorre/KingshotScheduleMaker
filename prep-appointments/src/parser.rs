...
pub fn has_player_submitted<P: AsRef<Path>>(csv_path: P, player_id: &str) -> bool {
    ...
    let mut reader = match Reader::from_path(csv_path) {
        Ok(r) => r,
        Err(_) => return false,
    };
    ...
    let id_col = headers
        .iter()
        .position(|h| h.contains("player ID"))
        .unwrap_or(4);
    for result in reader.records() {
        ...
        if let Some(id) = record.get(id_col) {
            if id.trim() == player_id {
                return true;
            }
        }
    }
...
fn time_string_to_slot_number(time_str: &str, time_slots: &[(u8, String)]) -> Option<u8> {
    // Remove any notes or extra text in parentheses
    let clean_time = time_str.split('(').next().unwrap_or(time_str).trim();
...
fn parse_time_slots(time_string: &str, custom_time_slots: Option<&[(u8, String)]>) -> Vec<u8> {
...
pub fn load_appointments<P: AsRef<Path>>(
    ...
    // Find column indices
    let alliance_col = headers
        .iter()
        .position(|h| h.contains("alliance"))
        .unwrap_or(1);
    let custom_alliance_col = headers
        .iter()
        .position(|h| h.contains("Non of the above") && h.contains("type it here"))
        .unwrap_or(2);
    let name_col = headers
        .iter()
        .position(|h| h.contains("character name"))
        .unwrap_or(3);
    let id_col = headers
        .iter()
        .position(|h| h.contains("player ID"))
    ...
    let submission_type_col = headers
        ...
        .unwrap_or(5);
    let construction_want_col = headers
        .iter()
        .position(|h| h.contains("Construction day appointment"))
        .unwrap_or(6);
    let construction_speedups_col = headers
        .iter()
        .position(|h| h.contains("Construction day") && h.contains("speedups"))
        .unwrap_or(7);
    let construction_truegold_col = headers
        .iter()
        .position(|h| {
            h.contains("truegold") && !h.contains("dust") && !h.to_lowercase().contains("tempered")
        })
        .unwrap_or(8);
    let construction_tempered_truegold_col = headers
        .iter()
        .position(|h| h.to_lowercase().contains("tempered") && h.contains("truegold"));
    let construction_times_col = headers
        .iter()
        .position(|h| h.contains("Construction day appointment") && h.contains("times"))
        .unwrap_or(9);
    let research_want_col = headers
        .iter()
        .position(|h| h.contains("Research day appointment") && !h.contains("times"))
        .unwrap_or(10);
    let research_speedups_col = headers
        .iter()
        .position(|h| h.contains("Research day") && h.contains("speedups"))
        .unwrap_or(11);
    let research_truegold_dust_col = headers
        .iter()
        .position(|h| h.contains("truegold dust"))
        .unwrap_or(12);
    let research_times_col = headers
        .iter()
        .position(|h| h.contains("Research day appointment") && h.contains("times"))
        .unwrap_or(13);
    let troops_want_col = headers
        .iter()
        .position(|h| h.contains("Troops Training day appointment") && !h.contains("times"))
        .unwrap_or(13);
    let troops_speedups_col = headers
        .iter()
        .position(|h| h.contains("Troops Training day") && h.contains("speedups"))
        .unwrap_or(14);
    let troops_times_col = headers
        .iter()
        .position(|h| h.contains("Troops Training day appointment") && h.contains("times"))
        .unwrap_or(15);
    ...
    for result in reader.records() {
        ...
        {
            let custom_alliance = record
                .get(custom_alliance_col)
                .unwrap_or("")
                .trim()
                .to_string();
        ...
        let is_resubmission =
            submission_type.contains("re-submission") || submission_type.contains("resubmission");
        ...
        let construction_speedups =
            parse_number(record.get(construction_speedups_col).unwrap_or(""));
        ...
        let construction_truegold =
            parse_number(record.get(construction_truegold_col).unwrap_or(""));
        ...
        let research_truegold_dust =
            parse_number(record.get(research_truegold_dust_col).unwrap_or(""));
        ...
        let construction_available_slots =
            parse_time_slots(construction_times, construction_time_slots);
